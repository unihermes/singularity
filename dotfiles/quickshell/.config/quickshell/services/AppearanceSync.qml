...
// accent colours, in a marked block of their user.js (browserBlock). The
// rest of each user.js is left alone. Read at each browser start.
//
...
Scope {
    ...
    readonly property string browserBegin: "// neutrino: begin -- generated by quickshell AppearanceSync, edits are overwritten"
    readonly property string browserEnd: "// neutrino: end"

    function browserBlock() {
        var a = hex(Theme.accent), t = hex(onAccent())
        var pref = (k, v) => 'user_pref("' + k + '", "' + v + '");'
        return [browserBegin,
            pref("ui.highlight", a), pref("ui.highlighttext", t),
            pref("ui.selecteditem", a), pref("ui.selecteditemtext", t),
            pref("ui.accentcolor", a), pref("ui.accentcolortext", t),
            browserEnd].join("\n")
    }
    ...
    function renderBrowsers() {
        var block = browserBlock()
        var re = new RegExp("\\n?" + browserBegin.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")
            + "[\\s\\S]*?" + browserEnd + "\\n?")
